...
from SPcv.speval import COCOExpandEval
from SPcv.location_evaluation import LocationEvaluator
from functools import partial


@DATASETS.register_module()
class SeaPersonDataset(CustomDataset):
...
# add by hui
    def __init__(self,
        ...
        super(SeaPersonDataset, self).__init__(
            ann_file,
            data_root=data_root,
            **kwargs
        )
            ...
            for i, img_info in enumerate(self.data_infos):
                ...
                for ann in self.coco.imgToAnns[img_id]:
                    if 'ignore' in ann and ann['ignore']:
                        continue
                    if min(img_info['width'], img_info['height']) >= min_size and ann['bbox'][-1] > self.min_gt_size and \
                            ann['bbox'][-2] > self.min_gt_size:
                        valid = True
                if valid:
                    valid_inds.append(i)
...
# change by hui
    def evaluate(self,
                 ...
                 cocofmt_kwargs={},  # add
                 skip_eval=False,  # add
                 use_location_metric=False,  # add
                 location_kwargs={},  # add
                 use_without_bbox_metric=False,  # add
                 without_bbox_kwargs={},  # add
                 save_result_file=None):  # add
        ...
        for metric in metrics:
            ...
            try:
                ...
                if not os.path.exists(os.path.dirname(save_result_file)):
                    os.makedirs(os.path.dirname(save_result_file))
